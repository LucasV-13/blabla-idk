document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Tentative de jeu de carte:', cardId, cardValue);
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // Si erreur (cartes plus petites existaient), gérer la perte de vie
            if (data.error_card) {
                console.log('❌ Erreur de carte - Tout le monde perd une vie !');
                ...
                if (hearts.length > 0) {
                    ...
                    lastHeart.style.animation = 'heartBeat 1s ease-in-out';
                    
                    // Effet de secousse sur l'affichage des vies
                    const viesDisplay = document.querySelector('.lives-display');
                    if (viesDisplay) {
                        viesDisplay.style.animation = 'shake 0.6s ease-in-out';
                    }
                }
                
                // Afficher un message détaillé
                if (data.plus_petite_carte_en_jeu) {
                    showTemporaryMessage(
                        `💔 Erreur ! Il restait la carte ${data.plus_petite_carte_en_jeu} en jeu. Tout le monde perd une vie !`,
                        'error',
                        4000
                    );
                }
                ...
                if (data.partie_status === 'terminee') {
                ...
                playSound('success');
                showTemporaryMessage(
                    `✅ Carte ${data.carte_jouee} jouée avec succès !`,
                    'success',
                    2000
                );
            }
            ...
            if (data.partie_status === 'niveau_termine') {
                setTimeout(() => {
                    showLevelCompleteModal({
                        niveau_termine: gameState.niveau,
                        prochain_niveau: gameState.niveau + 1
                    });
                    playSound('level_complete');
                }, 1000);
            } else if (data.partie_status === 'gagnee') {
                setTimeout(() => {
...
// AJOUTEZ cette nouvelle fonction après la fonction playCard :
function showTemporaryMessage(message, type = 'info', duration = 3000) {
    ...
    messageDiv.className = `temporary-message temporary-${type}`;
...
// Ajouter les animations CSS pour les messages temporaires
