...
$texts = [
    ...
    'lower_card_exists' => 'Une carte plus petite existe chez un autre joueur',
...
try {
    ...
    // ÉTAPE 2: Vérifier s'il existe une carte plus petite que celle jouée chez TOUS les joueurs
    $cartesPlusPetitesQuery = "SELECT MIN(valeur) as min_valeur, COUNT(*) as nb_cartes_plus_petites
                               FROM Cartes 
                               WHERE id_partie = :partie_id 
                               AND etat = 'en_main' 
                               AND valeur < :valeur_carte_jouee";
    ...
    error_log("play_card.php - Vérifications: carte_jouee=$valeurCarteJouee, derniere_jouee=$maxValeurJouee, nb_plus_petites=$nbCartesPlusPetites, min_en_jeu=$minValeurEnJeu");
    
    // Commencer une transaction
    $conn->beginTransaction();
    
    // Déterminer s'il y a une erreur (carte plus petite en jeu)
    $erreurCarte = ($nbCartesPlusPetites > 0);
    
    if ($erreurCarte) {
        // Il existe une ou plusieurs cartes plus petites en jeu
        ...
        error_log("play_card.php - Erreur de carte, vie perdue. Carte jouée: $valeurCarteJouee, Plus petite carte en jeu: $minValeurEnJeu, Vies restantes: $viesRestantes");
        ...
        if ($viesRestantes <= 0) {
            ...
            // Enregistrer les scores de fin de partie
            try {
                $saveScoreQuery = "INSERT INTO Actions_jeu (id_partie, id_utilisateur, type_action, details) 
                ...
                $saveScoreStmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
                $detailsFinPartie = json_encode(['resultat' => 'defaite', 'niveau_atteint' => $partie['niveau']]);
                $saveScoreStmt->bindParam(':details', $detailsFinPartie, PDO::PARAM_STR);
                $saveScoreStmt->execute();
            } catch (PDOException $e) {
                error_log("play_card.php - Erreur lors de l'enregistrement des scores: " . $e->getMessage());
            }
        }
    }
    
    // Jouer la carte (mettre à jour son état) - TOUJOURS jouer la carte même en cas d'erreur
    ...
    // Ne vérifier la fin de niveau que si on n'a pas perdu
    if ($cartesRestantes === 0 && $partie['vies_restantes'] > 0) {
        ...
        if ($partie['niveau'] >= 12) {
            ...
            try {
                ...
                $saveScoreStmt->bindParam(':details', $detailsVictoire, PDO::PARAM_STR);
                $saveScoreStmt->execute();
            } catch (PDOException $e) {
                error_log("play_card.php - Erreur lors de l'enregistrement du score de victoire: " . $e->getMessage());
            }
        } else {
    ...
    $successMessage = $erreurCarte ? 
        $texts['card_played_error'] . " (Plus petite carte en jeu: $minValeurEnJeu)" : 
