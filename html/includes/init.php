...
set_error_handler(function($severity, $message, $file, $line) {
    ...
    // Empêcher l'affichage d'erreurs qui peuvent casser les headers
    return true;
});
...
class Init {
     ...
     * CORRECTION : Gestion appropriée des redirections
     */
    public static function protectedPage($redirectUrl = null) {
    // URL de redirection par défaut vers la page de connexion
    if (!$redirectUrl) {
        $redirectUrl = SITE_URL . 'index.php';
    }
    
    // Vérifier l'authentification SANS redirection automatique
    $isAuthenticated = Security::checkAuthentication(false);
    
    if (!$isAuthenticated) {
        // Log pour debug
        if (class_exists('Logger')) {
            Logger::info('Accès refusé - non authentifié', [
                'page' => $_SERVER['REQUEST_URI'] ?? '',
                'session_data' => [
                    'has_user_id' => isset($_SESSION['user_id']),
                    'has_username' => isset($_SESSION['username']),
                    'has_expires' => isset($_SESSION['expires']),
    ...
    // L'utilisateur est authentifié - prolonger la session
    $_SESSION['expires'] = time() + SESSION_LIFETIME;
    
    // Log de l'accès réussi
    if (class_exists('Logger')) {
        Logger::info('Page protégée accédée avec succès', [
            'page' => $_SERVER['REQUEST_URI'] ?? '',
    ...
    public static function adminPage($redirectUrl = null) {
        if (!$redirectUrl) {
            $redirectUrl = SITE_ROOT . 'pages/dashboard.php';
        }
    ...
    public static function gamePage($gameId = null) {
        ...
        if ($gameId && function_exists('gameDb')) {
            $gameDb = gameDb();
            
            // Vérifier que la partie existe
            $game = $gameDb->getGameInfo($gameId);
            if (!$game) {
                header("Location: " . SITE_ROOT . 'pages/dashboard.php');
                exit();
            }
...
function generateMeta($title = null, $description = null, $keywords = null) {
    $siteName = Language::get('site_title', 'The Mind');
