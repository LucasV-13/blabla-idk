...
class Security {
    ...
    public static function checkAuthentication($redirectOnFail = true) {
        ...
        if (!$isAuthenticated) {
            if ($redirectOnFail) {
                self::destroySession();
                self::redirect(getUrl('index.php'));
            }
            return false;
        }
    ...
    public static function checkPermission($requiredRole, $redirectOnFail = true) {
        ...
        $hasPermission = ($userRole === strtolower($requiredRole)) || 
                        ($userRole === 'admin'); // Les admins ont tous les droits
        
        if (!$hasPermission && $redirectOnFail) {
            self::redirect(getUrl('pages/dashboard.php'));
        }
